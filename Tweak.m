...
/*
 * OBD11 / OBDeleven update bypass for rootful iOS 14.
 * Uses the same working runtime version/build/header spoofing as the iOS 16 build,
 * with rootful preference paths and no Dopamine/libSandy dependency.
 */
...
static NSString *const kRootfulPreferencesPath =
...
static NSString *sanitizedVersionString(id candidate) {
    ...
    if (trimmed.length == 0 || trimmed.length > 32) return nil;

    NSCharacterSet *allowed = [NSCharacterSet characterSetWithCharactersInString:@"0123456789."];
    if ([trimmed rangeOfCharacterFromSet:[allowed invertedSet]].location != NSNotFound) return nil;
    if ([trimmed rangeOfCharacterFromSet:[NSCharacterSet decimalDigitCharacterSet]].location == NSNotFound) return nil;

    return trimmed;
}

static NSDictionary *preferencesDictionaryFromDisk(void) {
    NSDictionary *dictionary = [NSDictionary dictionaryWithContentsOfFile:kRootfulPreferencesPath];
...
static BOOL spoofingEnabled(void) {
    @synchronized ([NSBundle class]) {
        ...
        if (gActualShortVersion.length > 0 && [selected isEqualToString:gActualShortVersion]) return NO;
        return YES;
    }
}
...
static void preferencesChanged(CFNotificationCenterRef center,
    ...
    @autoreleasepool {
        reloadPreferences();
        NSLog(@"[OBDelevenUpdateBypass] prefs reloaded; app=%@ enabled=%d selected=%@ actual=%@ active=%d",
              gIsVAG ? @"VAG" : @"OBD11",
              gEnabled,
...
#pragma mark - NSBundle version/build spoof
...
static id spoofedObjectForInfoKey(NSBundle *self, SEL _cmd, NSString *key) {
    if (self == gMainBundle && spoofingEnabled()) {
        if ([key isEqualToString:@"CFBundleVersion"]) return kWorkingSpoofedBuildVersion;
        if ([key isEqualToString:@"CFBundleShortVersionString"]) return currentSpoofedShortVersion();
    }
...
static NSDictionary *spoofedInfoDictionary(NSBundle *self, SEL _cmd) {
    ...
    if (self != gMainBundle || !original || !spoofingEnabled()) return original;
...
static CFTypeRef spoofedCFBundleGetValue(CFBundleRef bundle, CFStringRef key) {
    if (bundle == CFBundleGetMainBundle() && key && spoofingEnabled()) {
        if (CFEqual(key, CFSTR("CFBundleVersion"))) return (__bridge CFTypeRef)kWorkingSpoofedBuildVersion;
        if (CFEqual(key, CFSTR("CFBundleShortVersionString"))) return (__bridge CFTypeRef)currentSpoofedShortVersion();
    }
    return originalCFBundleGetValue(bundle, key);
}

#pragma mark - Request header spoof

static NSURLRequest *requestBySpoofingVersionHeader(NSURLRequest *request) {
    if (!request || !spoofingEnabled()) return request;
...
static NSURLSessionDataTask *spoofedDataTaskRequestCompletion(NSURLSession *self, SEL _cmd, NSURLRequest *request, void (^completion)(NSData *, NSURLResponse *, NSError *)) {
    return originalDataTaskRequestCompletion(self, _cmd, requestBySpoofingVersionHeader(request), completion);
}
...
static NSURLSessionDataTask *spoofedDataTaskRequest(NSURLSession *self, SEL _cmd, NSURLRequest *request) {
...
static NSURLSessionUploadTask *spoofedUploadTaskDataCompletion(NSURLSession *self, SEL _cmd, NSURLRequest *request, NSData *bodyData, void (^completion)(NSData *, NSURLResponse *, NSError *)) {
    return originalUploadTaskDataCompletion(self, _cmd, requestBySpoofingVersionHeader(request), bodyData, completion);
}
...
static void OBDelevenUpdateBypassInit(void) {
    @autoreleasepool {
        ...
        NSLog(@"[OBDelevenUpdateBypass] rootful iOS14 v1.0.0 loaded; app=%@ enabled=%d selected=%@ actual=%@ active=%d build=%@ header=%@",
              gIsVAG ? @"VAG" : @"OBD11",
              gEnabled,
              currentSpoofedShortVersion(),
              gActualShortVersion,
